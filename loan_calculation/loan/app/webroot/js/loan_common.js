...
useddata[24] = {"yeardiff":5,"maxlptrate":50,"minlptrate":5,"minlpprate":65};
useddata[36] = {"yeardiff":5,"maxlptrate":40,"minlptrate":5,"minlpprate":55};
useddata[48] = {"yeardiff":4,"maxlptrate":30,"minlptrate":5,"minlpprate":45};
useddata[60] = {"yeardiff":3,"maxlptrate":20,"minlptrate":5,"minlpprate":35};
for(i=13;i<=23;i++){
...
*/
				...
				if(bptmax < 0){
					...
					/*
						...
						中古車WP・SWP　→　Excel版に掲載の回数別の車両本体×パーセンテージ + 元本充当額ガード									
				...
				if(useddata[chg_installments(installments)]['yeardiff'] > getYearDiff($("#tourokuyear").val(),$("#tourokumonth").val(),$("#usedcar_year").val(),$("#usedcar_month").val())){
					...
					if(bptmax < 0){
						...
						*/
						...
						// さらにガードをする
						// 全てのファイナンスプラン　→　月額3000円未満ガード
						if(resultArr['monthlypayment'] < 3000 || resultArr['loanprincipal']<500000){
							guard_flg = 1;		// 計算NG
						}else{
